...
def connections(tangents,
    ...
    edge_index = np.vstack([csgraph.tocoo().row, csgraph.tocoo().col])
...
def expand_edge_index(edge_index, dim=1):
    ...
    ind = [np.array([i, j]) for i in range(dim) for j in range(dim)]
    edge_index = [edge_index * dim + i[:,None] for i in ind]
    edge_index = np.stack(edge_index, axis=2).reshape(2, n * len(ind))
