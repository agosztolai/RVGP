...
#from misc import smooth, subsample, get_vector_field
#from design_input import design_input
...
def move_to_gpu(net, x, inp=None):
...
class timeseries(Dataset):
    ...
    def __getitem__(self, idx):
        if self.inp is None:
            ...
            return self.x[idx], self.inp[idx]
...
def plot_trajectories(net, trajectories, ax, n_traj=2, style="-", c="C0", interval=[None, None]):
    ...
    m1 = net.m[:, 0].detach().cpu().numpy()
    m2 = net.m[:, 1].detach().cpu().numpy()
...
def plot_field_noscalings(
    ...
    if vec1 is None:
        vec1 = net.m[:, 0].squeeze().detach().cpu().numpy()
    if vec2 is None:
        vec2 = net.m[:, 1].squeeze().detach().cpu().numpy()
    if add_fixed_points:
        n1 = net.n[:, 0].squeeze().detach().cpu().numpy()
        n2 = net.n[:, 1].squeeze().detach().cpu().numpy()
    m = net.m.detach().cpu().numpy()
    n = net.n.detach().cpu().numpy()
