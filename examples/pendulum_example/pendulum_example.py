...
ax.quiver(X[:,0], X[:,1],X[:,2], f[:,0], f[:,1], f[:,2], color='b', length=1)
...
vector_field_GP = gpflow.models.GPR((x_train_f, f_train), kernel=kernel, noise_variance=0.01)
...
ax.quiver(X[:,0], X[:,1], X[:,2], f[:,0], f[:,1], f[:,2], color='r', length=2)
...
manifold_GP_inv = gpflow.models.GPR((x_train, y_train), kernel=gpflow.kernels.RBF(), noise_variance=1)
#manifold_GP_inv = gpflow.models.GPR((x_train, y_train), kernel=kernel, noise_variance=0.01)
...
ax.quiver(X[:,0], X[:,1],X[:,2],  f_pred_mean[:,0], f_pred_mean[:,1], f_pred_mean[:,2], color='r', length=2)
#ax.axis('equal')
...
for i in range(len_t):
    ...
    f_pred, f_var = vector_field_GP.predict_f(x_.numpy().reshape(X.shape[1],50))
