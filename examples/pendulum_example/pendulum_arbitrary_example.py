...
#%% embed system on parabola

def parabola(X, Y, alpha=0.5):
    Z = -(alpha*Y)**2# - (alpha*X)**2    
    return np.column_stack([X.flatten(), Y.flatten(), Z.flatten()])

from scipy.stats import norm
def wavey_manifold(X,Y):
    Z = 0.5*(np.sin(X) + np.sin(Y))
    return np.column_stack([X.flatten(), Y.flatten(), Z.flatten()])

#x = parabola(positions , momentums)
...
rollouts_ = rollouts.reshape([rollouts.shape[0]*rollouts.shape[1],rollouts.shape[2]])
rollouts_3d = wavey_manifold(rollouts_[:,0],rollouts_[:,1])
rollouts_3d = rollouts_3d.reshape([rollouts.shape[0],rollouts.shape[1],3])
...
sample_ind, _ = furthest_point_sampling(X, stop_crit=0.04) #uniform sampling of the datapoints
X = X[sample_ind]
f = f[sample_ind]
dim_emb = X.shape[1]
...
ax = graph(G_)
ax.quiver(X[:,0], X[:,1], X[:,2], f[:,0], f[:,1], f[:,2], color='b', length=10)
ax.axis('equal')
...
ax = graph(G_)
ax.quiver(X[:,0], X[:,1], X[:,2], f[:,0], f[:,1], f[:,2], color='g', length=10)
...
ax = graph(G_)
ax.quiver(X[:,0], X[:,1], X[:,2], f[:,0], f[:,1], f[:,2],  color='g', length=10)
ax.quiver(X[:,0], X[:,1], X[:,2], f_pred_mean[:,0], f_pred_mean[:,1], f_pred_mean[:,2], color='r', length=10)
ax.axis('equal')
...
for i in range(len_t):
    ...
    y1 = y + h * f_pred    #     y1 = y_pred + h * f_pred    
...
for i in range(0,len_t,3):
    y = trajectory[i]
    f_traj = vectors[i]
    ax.quiver(y[0], y[1], y[2],  f_traj[0,0], f_traj[0,1],  f_traj[0,2], color='r',length=10)
