function [clusters, p_values, t_sums, permutation_distribution ] = permutest_fullperm( trial_group_1, trial_group_2, dependent_samples, ...
...
for p = 1:num_permutations
    ...
    % Shuffle bins
    flat = t_value_vector(:);
    idx = randperm(length(flat));
    t_value_vector = reshape(flat(idx),size(t_value_vector,1),size(t_value_vector,2)); 
